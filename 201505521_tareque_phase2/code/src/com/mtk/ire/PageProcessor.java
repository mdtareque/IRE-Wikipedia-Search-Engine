...
*/	//		writePageToFile(p);
		if(dict.size() > INFO.INDEX_SPLIT_BLOCK) {
			try {
				Indexer.writeIndexToFile(TitleIndexCreator.bw);
			} catch (IOException e) {
				e.printStackTrace();
			}
			Indexer.words.clear();
...
//		if(!INFO.WRITE_MAIN_INDEX) return"";
		...
		for (String t : tokens) {
//			if (t.length() < 3) // skip 0,1,2 length words  // for titles don't skip, single separate run
//				continue;
//			if (!stopWords.contains(t)) { // no stemming titles
				stemmed = getStem(t);
